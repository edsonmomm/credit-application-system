...
class CustomerControllerTest {
  ...
  fun `should create a customer and return 201 status`() {
    ...
    mockMvc.perform(
      ...
      .andExpect(MockMvcResultMatchers.jsonPath("$.firstName").value("Customname"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.lastName").value("Customlastname"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.cpf").value("28475934625"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.email").value("customer1@mailtest.com"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.income").value("1000.0"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.zipCode").value("000000"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.street").value("Street Fighter 32"))
  ...
  fun `should not find customer with invalid id and return 400 status`() {
    ...
    mockMvc.perform(
      ...
      .andExpect(
        MockMvcResultMatchers.jsonPath("$.exception")
          .value("class me.study.credit.application.exception.BusinessException")
      )
      .andExpect(MockMvcResultMatchers.jsonPath("$.details[*]").isNotEmpty)
      .andDo(MockMvcResultHandlers.print())
  }
  ...
  fun `should delete customer by id and return 204 status`() {
    ...
    //then
    mockMvc.perform(
      MockMvcRequestBuilders.delete("$URL?customerId=${customer.id}")
        .accept(MediaType.APPLICATION_JSON)
    )
  ...
  fun `should not delete customer by id and return 400 status`() {
    ...
    //then
    mockMvc.perform(
      MockMvcRequestBuilders.delete("$URL?customerId=${invalidId}")
        .accept(MediaType.APPLICATION_JSON)
    )
      .andExpect(MockMvcResultMatchers.status().isBadRequest)
      .andExpect(MockMvcResultMatchers.jsonPath("$.title").value("Bad Request! Consult the documentation"))
  ...
  fun `should update a customer and return 200 status`() {
    ...
    mockMvc.perform(
      ...
      .andExpect(MockMvcResultMatchers.status().isOk)
      .andExpect(MockMvcResultMatchers.jsonPath("$.firstName").value("CustomnameUpdate"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.lastName").value("CustomlastnameUpdate"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.cpf").value("28475934625"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.email").value("customer1@mailtest.com"))
  ...
  private fun builderCustomerDto(
    firstName: String = "Customname",
    lastName: String = "Customlastname",
    cpf: String = "28475934625",
    email: String = "customer1@mailtest.com",
    income: BigDecimal = BigDecimal.valueOf(1000.0),
    password: String = "1234",
    zipCode: String = "000000",
    street: String = "Street Fighter 32",
  ) = CustomerDTO(
    firstName = firstName,
    lastName = lastName,
    cpf = cpf,
  ...
  private fun builderCustomerUpdateDto(
    firstName: String = "CustomnameUpdate",
    lastName: String = "CustomlastnameUpdate",
